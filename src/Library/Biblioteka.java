...
public class Biblioteka {
    ...
    public void izdajKnjigu(Clan clan, String imeKnjige) {
        System.out.println("****************************");
        System.out.println("Izdavanje knjige");
        Knjiga knjiga = null;
    ...
    public void vratiKnjigu(Clan clan, String imeKnjige) {
        System.out.println("****************************");
        System.out.println("Vracanje knjige");
        ...
        if (!knjiga.isIzdata()) {
            System.out.println("Knjiga " + imeKnjige + " nije izdata.");
        } else {
            ArrayList<Knjiga> knjigeKodClana= clan.getPozajmnjeneKnjige();
            if (knjigeKodClana.size() > 0) {
                Knjiga knjigaKodClana = null;
                for (int i = 0; i < knjigeKodClana.size(); i++) {
                    knjigaKodClana = knjige.get(i);
                    if (knjigaKodClana.getNaslov().equals(imeKnjige)) {
                        break;
                    }
                }
                knjiga.setIzdata(false);
                clan.vrati(knjiga);
                System.out.println(knjiga.getNaslov() + " je upravo vracena od clana: " + clan.getIme());
            }
            else {
                System.out.println("Clan biblioteke " + clan.getIme() + " već nema iznajmljenih knjiga");
            }
        }
    }
    ...
    public void pregledZaduzenihKnjiga() {
        System.out.println("****************************");
        System.out.println("Pregled zaduzenih knjiga:");
    ...
    public void listaClanovaSaZaduzenimKnjigama() {
        System.out.println("****************************");
        System.out.println("Lista clanova sa zaduzenim knjigama:");

        for (int i = 0; i < clanovi.size(); i++) {
            if (clanovi.get(i).getPozajmnjeneKnjige().size()>0) {
                System.out.println(clanovi.get(i).getIme());
            }
        }
    }
